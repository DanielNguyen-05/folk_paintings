# researcher.py
# researcher.py (Phiên bản nâng cấp dùng Tavily)
...
# Lấy key từ biến môi trường (nhớ thêm vào .env: TAVILY_API_KEY=tvly-...)
...
def search_tavily_context(query: str) -> str:
    ...
    try:
        # search_depth="advanced" giúp lấy nội dung sâu và chất lượng hơn
...
# def search_wikipedia(query: str) -> str:
#     """Tìm và lấy nội dung chi tiết từ Wikipedia."""
#     try:
#         # Tìm page chính xác
#         results = wikipedia.search(query)
#         if not results: return ""
        
#         # Lấy toàn bộ nội dung (content) thay vì chỉ summary
#         # Server xử lý được text dài, không lo.
#         page = wikipedia.page(results[0])
#         return f"--- NGUỒN WIKIPEDIA: {page.title} ---\n{page.content[:2000]}...\n(Đã cắt bớt để vừa context)\n"
#     except Exception:
#         return ""

# def search_web_ddg(query: str, max_results=3) -> str:
#     """Tìm kiếm Google/DuckDuckGo và lấy nội dung."""
#     text_content = ""
#     try:
#         ddgs = DDGS()
...
#         if results:
#             text_content += "--- NGUỒN WEB (DUCKDUCKGO) ---\n"
#             for res in results:
#                 text_content += f"Tiêu đề: {res['title']}\nLink: {res['href']}\nTóm tắt: {res['body']}\n\n"
#     except Exception as e:
...
#         wiki_future = loop.run_in_executor(pool, search_wikipedia, query)
#         web_future = loop.run_in_executor(pool, search_web_ddg, query)
        
#         wiki_res, web_res = await asyncio.gather(wiki_future, web_future)
